...
Particle.prototype.step = function() {
    ...
    ctx.strokeStyle = this.color.replace('hue', this.tick +
        (tick - this.tick) * opts.colorAttenuator).
        replace('alp', opts.baseAlpha + opts.addedAlpha * Math.random());
    ctx.beginPath();
    ...
    for (var i = 0; i < num; ++i) {
        ...
        ctx.lineTo(prevPointX + (Math.random() - .5) * added |
            0, prevPointY + (Math.random() - .5) * added | 0);
    }
    ctx.stroke();
